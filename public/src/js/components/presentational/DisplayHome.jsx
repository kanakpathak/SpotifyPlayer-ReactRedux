import React from "react";
import Button from '@material-ui/core/Button';
import {Link} from 'react-router-dom';

const DisplayHome = ({accessToken, searchArtistName, name, artist})=>{
    return(
        <div>
            {/* <Link to = '/login'>Welcome to Spotify Player</Link> */}
            <input type="text" name="search_box" onChange={searchArtistName} value={name} />
           {/* <p>
           TOKEN: {accessToken}
           </p> */}
           {/* <p>ARTIST: {name}</p> */}
           <ul>
               {artist.map((item, index)=>
                    <li key={index}>
                        <b>Artist: </b>{item['name']} <br/>
                        <b>followers: </b>{item['followers']['total']} <br/>
                        <b>Genre : </b> {item['genres'].map(itr=>itr+' ')} <br/>
                        <b>Popularity: </b>{item['popularity']}<br/>
                        <img src={item['images'].length>0? item['images'][0]['url']: 'data:image/png;base64,'} alt="" height='42' width='42'/> 

                        
                    </li>
                    )
                }
           </ul>

            
        </div>
    );
}

export default DisplayHome;